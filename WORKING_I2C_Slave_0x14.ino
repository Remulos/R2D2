...
char sendStatus[31]  = "111222333444555666777888999000";
int index = 0;
char pwm[15] = "00000000000000";
...
void receiveEvent(int byteCount){
  ...
  if (String(command[1]) != "A" ) {
    pinVal = pin.toInt(); // in case of not an analog pin assignment convert into digital pin number
  }
  if (String(command[1]) != "A" ) {
    pwm[pinVal] = 0; // in case of not an analog pin assignment set PWM flag to 0
  }

  // incase of analog pin assignment determine analog pin to be set
  if (String(command[1]) == "A" && String(command[2]) == "0") {
    pinVal = A0;
  }
  if (String(command[1]) == "A" && String(command[2]) == "1") {
    pinVal = A1;
  }
  if (String(command[1]) == "A" && String(command[2]) == "2") {
    pinVal = A2;
  }
  if (String(command[1]) == "A" && String(command[2]) == "3") {
    pinVal = A3;
  }

  // if requested set pinmode
  if (String(command[0]) == "S" && String(command[3]) == "I") {
    pinMode(pinVal, INPUT);
  }
  if (String(command[0]) == "S" && String(command[3]) == "O") {
    pinMode(pinVal, OUTPUT);
  }
  if (String(command[0]) == "S" && String(command[3]) == "P") {
    pinMode(pinVal, INPUT_PULLUP);
  }

  
  // if requested perform digital write
  if (String(command[0]) == "W" && String(command[3]) == "H") {
    digitalWrite(pinVal, HIGH);
  }
  if (String(command[0]) == "W" && String(command[3]) == "L") {
